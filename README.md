...
./serve.sh --dev     # developers: adds the in-game debug console — press [`]
                     # (dev-server-gated; it does not exist in public builds)
```
...
And words are doors: when a resident mentions a real place, a real person, or one of the desert's ideas — the Rust, the chorus, the hollow places — the words **glow**. Click one to ask about it, and it joins a persistent list of subjects (the SPEAK OF rail) you can raise with anyone you ever meet. Who can speak to what is resolved over the same blackboard of facts: a scavver gives true bearings to ruins inside her mental map and shrugs at ones beyond it, each in her own temperament's voice — and the ones who trust you say more.
...
And the big ruins are no longer only exteriors: every megastructure keeps a sealed threshold at its foot. Break the seal and **the hollow places** open — a warren of buried halls grown on the spot from a room grammar keyed to the structure's type (a colossus opens into plating galleries and a core shrine; a ring into cargo halls and the last platform). Down there the wind dies, the sky is a rumor, dead lamps still draw on some deep reserve, and scrap sits where no salvage crew ever reached. The halls descend 2–3 floors by stair galleries, the deep rooms hold sealed caches with real parts, things that never left lurk in the dark, and documents testify about the building you're standing in. And when someone who *mattered* falls — a companion, a true friend — the well's rite is not enough: their pattern was woven on a **Neuromanifold Shaper**, and the last Shapers are kept in the conceptories, in the deep rooms, in the hands of the Mother who made them. She will not trade.
